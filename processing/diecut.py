...
def detect_sides(client, original_path, diecut_path, model="gemini-3-flash-preview"):
    """Ask a vision model to report left/right by position in the diecut image.

    Returns a side_map with left_x and right_x per pair (e.g. upperarm, forearm).
    Left = piece on the left side of the image (smaller x), right = piece on
    the right (larger x). We keep the same side as in the image; no flip for
    facing direction. The bbox step uses this to assign left_* / right_* labels.
    """
    original = Image.open(original_path)
    diecut_img = Image.open(diecut_path)
    ...
    prompt = """Look at Image 1 (the original character) and Image 2 (the diecut with separated pieces).

For each PAIR of body parts below, report the approximate pixel x-center of each piece in the DIECUT image (Image 2). Use only position in the image; keep the same side as in the image (do not flip based on facing direction).

Use only POSITION (x-coordinate) in the diecut image, not shape:
- left_x = x-center of the piece on the LEFT side of the image (smaller x).
- right_x = x-center of the piece on the RIGHT side of the image (larger x).

Pairs to identify (give x_center for each piece):
- upperarm (2 pieces)
- forearm (2 pieces)
- hand (2 pieces)
- thigh (2 pieces)
...
{
  "sides": {
    "upperarm": {"left_x": <x_center of left upperarm piece>, "right_x": <x_center of right upperarm piece>},
    "forearm": {"left_x": <x_center>, "right_x": <x_center>},
    "hand": {"left_x": <x_center>, "right_x": <x_center>},
    "thigh": {"left_x": <x_center>, "right_x": <x_center>},
    "calf": {"left_x": <x_center>, "right_x": <x_center>},
    "foot": {"left_x": <x_center>, "right_x": <x_center>}
  }
}

Use pixel x-coordinates in Image 2 (the diecut) so we can match bounding boxes to the correct side."""

    print("[detect_sides] Asking vision model to identify left/right sides...")
    try:
        response = client.models.generate_content(
            model=model,
            contents=[
                types.Part.from_bytes(
                    data=pil_to_bytes(original), mime_type="image/png"
                ),
                types.Part.from_text(text="Image 1: ORIGINAL CHARACTER"),
                types.Part.from_bytes(
                    data=pil_to_bytes(diecut_img), mime_type="image/png"
                ),
                types.Part.from_text(text="Image 2: DIECUT (separated pieces)"),
        ...
        print(f"[detect_sides] Got side mapping for {len(sides)} pairs: {sides}")
        return sides
    except Exception as e:
        print(f"[detect_sides] Failed: {e} — falling back to spatial detection")
        return None


EXPECTED_SIDE_PAIRS = ("upperarm", "forearm", "hand", "thigh", "calf", "foot")


def _validate_side_map(sides):
    """Return True if sides has all expected pairs with numeric left_x and right_x."""
    if not sides or not isinstance(sides, dict):
        return False
    for key in EXPECTED_SIDE_PAIRS:
        entry = sides.get(key)
        if not isinstance(entry, dict):
            return False
        lx, rx = entry.get("left_x"), entry.get("right_x")
...
def get_side_map_from_diecut_image(chat, diecut_image_bytes, diecut_mime):
    """Ask the diecut model (same chat) to label left/right by position in its image.

    Uses x-coordinate as the primary cue so we don't rely on shape similarity.
    Returns a side_map dict or None if parsing fails.
    """
    prompt = """This image is the diecut you just produced with 14 separated body pieces.

For each PAIR of body parts (upperarm, forearm, hand, thigh, calf, foot), report the approximate pixel x-center of each piece in the image.

CRITICAL — Use only POSITION (x-coordinate) in the image. Keep the same side as in the image; do not flip based on facing direction:
- left_x = x-center of the piece that is on the LEFT side of the image (smaller x).
- right_x = x-center of the piece that is on the RIGHT side of the image (larger x).
Do NOT use shape or appearance; use only horizontal position.

Return ONLY valid JSON, no other text, in this exact format:
{"sides": {
  "upperarm": {"left_x": <number>, "right_x": <number>},
  "forearm": {"left_x": <number>, "right_x": <number>},
  "hand": {"left_x": <number>, "right_x": <number>},
  "thigh": {"left_x": <number>, "right_x": <number>},
  "calf": {"left_x": <number>, "right_x": <number>},
  "foot": {"left_x": <number>, "right_x": <number>}
}}"""

    print("[diecut] Asking diecut model for side map (position-based)...")
    try:
        response = chat.send_message(
            [
                types.Part.from_bytes(
                    data=diecut_image_bytes,
                    mime_type=diecut_mime or "image/png",
                ),
                types.Part.from_text(text=prompt),
            ]
        )
        ...
        if not text:
            print("[diecut] No text in side-map response")
            return None
        ...
        sides = result.get("sides", result) if isinstance(result, dict) else {}
        if _validate_side_map(sides):
            print(f"[diecut] Got position-based side map for {len(sides)} pairs")
            return sides
        print("[diecut] Side map missing required keys or values")
        return None
    except Exception as e:
        print(f"[diecut] Side map from diecut model failed: {e}")
        return None
...
def diecut(
    ...
    example_input_path = os.path.join(
        os.path.dirname(__file__), "..", "resources", "tobyturtle.png"
    )
    example_output_path = os.path.join(
        os.path.dirname(__file__), "..", "resources", "tobyturtle-diecut2.png"
    )
    ...
    def judge_image(original_bytes, generated_bytes, generated_mime):
        ...
        for i, (name, criterion_prompt) in enumerate(JUDGE_CRITERIA, 1):
            ...
            try:
                ...
                resp = client.models.generate_content(
                    ...
                    contents=[
                        types.Part.from_bytes(
                            data=example_input_bytes, mime_type="image/png"
                        ),
                        types.Part.from_text(text="EXAMPLE CHARACTER"),
                        types.Part.from_bytes(
                            data=example_output_bytes, mime_type="image/png"
                        ),
                        types.Part.from_text(text="EXAMPLE GOOD DIECUT"),
                        types.Part.from_bytes(
                            data=original_bytes, mime_type="image/png"
                        ),
                        types.Part.from_text(text="ORIGINAL CHARACTER"),
                        types.Part.from_bytes(
                            data=generated_bytes,
                            mime_type=generated_mime or "image/png",
                        ),
        ...
        parts = [
            f"{name}:{'PASS' if s['pass'] else 'FAIL'}" for name, s in scores.items()
        ]
    ...
    for round_index in range(1, round_prompts + 1):
        if round_index == 1:
            print(
                f"[diecut] Round {round_index}/{round_prompts}: generating diecut with {model}..."
            )
            message_parts = [
                types.Part.from_text(text=prompt),
                types.Part.from_text(text="Example Input"),
                types.Part.from_bytes(
                    data=pil_to_bytes(example_input), mime_type="image/png"
                ),
                types.Part.from_text(text="Example Output"),
                types.Part.from_bytes(
                    data=pil_to_bytes(example_output), mime_type="image/png"
                ),
            ...
            print(
                f"[diecut] Round {round_index}/{round_prompts}: retrying with feedback..."
            )
            message_parts = [
        ...
        print(
            f"[diecut] Round {round_index}: got image, running judge ({judge_model})..."
        )
        ...
        failed_reasons = [f"- {s['reason']}" for s in scores.values() if not s["pass"]]
    ...
    if best_score == 5:
        ...
        print(
            f"[diecut] Best: round {best_round} ({best_score}/5 across {len(attempts)} attempt(s))"
        )
        if fail_on_review:
