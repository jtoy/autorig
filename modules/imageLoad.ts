...
let BASE_HOST: string = 'https://orchestrator.distark.com';
 ...
 */
export const setBaseHost = (host: string): void => {
    BASE_HOST = host;
};
 ...
 */
export const getBaseHost = (): string => BASE_HOST;
 ...
 */
export const resolveImageSource = (imageData: string | null | undefined): string | null => {
    ...
    if (md5Regex.test(imageData)) {
        return `${BASE_HOST}/api/v1/artifacts/${imageData}`;
    }
    ...
    if (truncatedHashRegex.test(imageData)) {
        ...
        return `${BASE_HOST}/api/v1/artifacts/${hash}`;
    }
...
export const loadImage = (
    ...
    const imageUrl = resolveImageSource(imageData);
 ...
 */
export const getImageCache = (): Map<string, HTMLImageElement> => IMAGE_CACHE;
 ...
 */
export const cacheImage = (key: string, image: HTMLImageElement): void => {
 ...
 */
export const loadAllRigImages = async (
    rigData: RigData,
    ...
    if (rigData.imagePaths) {
        for (const [key, hash] of Object.entries(rigData.imagePaths)) {
            if (hash && typeof hash === 'string' && !hash.includes('[MEDIA_REMOVED]')) {
                ...
                if (!useCache || !IMAGE_CACHE.has(imageKey)) {
